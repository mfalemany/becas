...
class ci_evaluacion_informes extends becas_ci
{
	...
	function conf__cu_informes(becas_ei_cuadro $cuadro)
	{
		...
		$informes = toba::consulta_php('co_informes')->get_informes_presentados($this->beca,TRUE);
		if($informes > 0){
	...
	function conf_evt__cu_informes__evaluar(toba_evento_usuario $evento,$fila)
	{
		...
		//$params[5] contiene "A" si el informe ya recibió tres o mas evaluaciones positivas
		//$params[5] es el estado de evaluacion: A es "Aprobado"
		if($params[5] == 'A'){
			$evento->desactivar();
		}else{
			$evento->activar();
	...
	{
		$informe = array('id_informe' => $seleccion['id_informe']);
		$this->get_datos()->cargar($informe);
	...
	{
		$fila = $this->get_datos('informe_evaluacion')->get_id_fila_condicion(array('nro_documento_evaluador'=>toba::usuario()->get_id()));
		//Si existe una evaluación, de este usuario para este informe...
		if(isset($fila[0])){
			$this->get_datos('informe_evaluacion')->set_cursor($fila[0]);
			$form->set_datos($this->get_datos('informe_evaluacion')->get());
		}
	}
