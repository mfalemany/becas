<div id="evaluacion_beca">
	<div id="datos_beca">
		<div>
			<span id='titulo_plan_beca'>Titulo del Plan de Beca</span>
		</div>
		<div>
			El Plan de Beca propuesto estará incluido en el siguiente Proyecto de Investigación Acreditado
			<span id='proyecto_acreditado'>Estudios de biología reproductiva comparada, ontogenia y desarrollo gonadal de vertebrados amniotas y anamniotas del Nordeste de Argentina.</span>
		</div>
		<div>
			Convocatoria: <span id='convocatoria'>Convocatoria a Becas CYT 2018</span>
			Tipo de Beca: <span id='tipo_beca'>PREGRADO</span>
			Área de Conocimiento: <span id='area_conocimiento'>Ciencias Exactas, Naturales y Agrimensura</span>
			Lugar de Trabajo: <span id='lugar_trabajo'>Facultad de Ciencias Agrarias</span>
			Área/Departamento: <span id='area_trabajo'>Departamento de Suelos</span>
		</div>
	</div>
	<div id="datos_postulante">
		
	</div>
	<div id="datos_direccion">
		
	</div>
	<div id="plan_trabajo">
		
	</div>
	<div id="antecedentes">
		
	</div>
</div>