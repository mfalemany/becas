...
class co_proyectos
{
	...
	{
		$filtro = quote("%{$filtro}%");
		$sql = "SELECT id_proyecto, proyecto FROM proyectos AS proy WHERE proyecto ILIKE $filtro LIMIT 20";
	...
	function get_proyecto_busqueda($id_proyecto)
	{
		...
		$sql = "SELECT proyecto FROM proyectos WHERE id_proyecto = $id_proyecto";
